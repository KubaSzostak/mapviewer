...
module metaViewer {
    ...
    function fillRecordsList(xmlDoc: Document) {
        ...
        var resCount = <any>jQuery(searchNode).attr("numberOfRecordsMatched");
        if (resCount <= 0) {
        ...
        (<any>jQuery("#records")).page(); //init page
    ...
    function detailsIdFromUrl() {
        ...
        var id = "";		
        if (strUtils.contains(window.location.hash, ("?"))) {
            ...
            novotive.io.getXml(url,
                function (responseXml) {
                    try {
						...
						var logErr = new novotive.log.LogRecord(e);
						logErr.setMessage("Unexpected error");
						console.error(logErr);
                        showErrorDialog(logErr);
                    }
                },
                function (error) {
                    error.setMessage("CSW service error");
    ...
    function getItemsTag(items: Array<any>, phraseId?: string) {
    ...
    function fillDetails(id: string, xmlDoc: Document) {
        ...
        try {
            ...
            var err = new novotive.log.LogRecord(e);
    ...
    function initDetailsComponents() {
        (<any>jQuery("#details")).page(); //init page
    ...
    function isShpUrl(url: string) {
        ...
        return strUtils.contains(geoserverShp, "shapefile", "shp.zip");
    }
